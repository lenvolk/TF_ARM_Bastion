...
then
    ...
    az storage account create --name $STORAGE_ACCOUNT_NAME --resource-group $RESOURCE_GROUP_NAME --location $REGION --sku Standard_LRS --kind StorageV2 --encryption-services blob
fi
...
terraform init -upgrade -input=false \
    ...
    -backend-config="key=$ENVIRONMENT-$APP_NAME"
